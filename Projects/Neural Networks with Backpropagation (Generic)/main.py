...
def create_or_get_datasets(index_x, index_y, axis=0, ignore_index=True, train_set_frac=1.0):
    if utils.are_sets_available(paths_to_sets):
        ...
        df_x['bias'] = np.ones((df_x.shape[0], 1))
...
def calculate_error_on_train_test(epoch, args):
    ...
    y_test_pred = __winner_takes_all_output(args['model'].predict(x_test, {}), y_test.index)
...
if __name__ == '__main__':
    ...
    feedFwdNwk = FeedForwardNetwork(layers=layers, neurons_in_layers=neurons_in_layers,
                                    model_args=model_args, inputs=features + 1, low=low, high=high,
                                    eta=0.1, alpha=0.1)
    ...
    performance = {}
    pre_args = {"x_test_set": X_test, "y_test_set": Y_test, "model": feedFwdNwk, "performance": performance,
                "when": "before"}
    ...
    epoch_post_args = {"error_frac": error_frac, "x_train": X_train, "y_train": Y_train,
                       "x_test": X_test, "y_test": Y_test, "model": feedFwdNwk, "epochs_interval": 10,
                       'low': low, 'high': high}
    ...
    weights = feedFwdNwk.train(X_train, Y_train, epochs=100,
    ...
    print("Plotting Error Fraction of the Neuron")
    ...
    y_pred = feedFwdNwk.predict(X_train, None)
    ...
    y_pred = feedFwdNwk.predict(X_test, None)
