...
class Perceptron(Classifier):
    ...
    def set_weighted_deltas(self, delta):
        self.weighted_delta = self.w.to_numpy() * delta

    def get_weighted_delta(self, index):
        return self.weighted_delta[index]


class Layer(Classifier):

    @staticmethod
    def __activation_function__(x, w):
        sig = 1 / (1 + np.exp(- np.dot(w, x.T)))
        return sig.T

    @staticmethod
    def __deriv_activation_function__(x, w):
        f_u = Layer.__activation_function__(x, w)
        return f_u * (1 - f_u)
    ...
    def __init__(self, eta, alpha, args):
        self.layer = args['layer']
        self.max_layer = args['max_layers']
        self.neurons_in_layer = args['neurons_in_layer']
        ...
        if 'weights' in args:
            ...
            self.w = args['w_gen'](Layer.__get_weights_range(args['w_gen_args']))
            self.w = self.w.reshape(self.neurons_in_layer, self.inputs)
        self.prev_delta = 0
        self.act_func = args['act_func'] if 'act_func' in args else Layer.__activation_function__
        self.deriv_act_func = args[
            'deriv_act_func'] if 'deriv_act_func' in args else Layer.__deriv_activation_function__
        self.input = None
        self.weighted_delta = []
    ...
    def predict(self, x, args):
        return Layer.__activation_function__(x, self.w)

    def __update_weights__(self, x=None, y=None, results=None, args=None):
        delta = args['delta']
        delta_w = np.dot((self.eta * delta).T, self.input)
        new_delta = delta_w + (self.alpha * self.prev_delta)
        self.w += new_delta
        self.prev_delta = new_delta
        return self.w
    ...
    def calc_delta_func(self, args):
        ...
        if args['is_hidden_layer']:
            ...
            sum_of_deltas = np.sum(w_i_j, axis=0)
            delta = sum_of_deltas * self.deriv_act_func(self.input, self.w)
        else:
            actual = args['actual']
            predict = args['predicted']
    ...
    def set_weighted_deltas(self, delta):
        deltas = np.array([d for d in delta for j in range(self.inputs)]).T
        self.weighted_delta = self.w * deltas

    def get_weighted_delta(self):
        return self.weighted_delta
...
class FeedForwardNetwork(Classifier):
    ...
    def __init__(self, layers=2, neurons_in_layers=None, inputs=None,
                 ...
                 low=0, high=0,
                 model_for_neurons=Layer, model_args=None,
        ...
        if not neurons:
            if inputs:
                ...
                for neuron_layer, layer in zip(neurons_in_layers, range(len(neurons_in_layers))):
                    ...
                    args.append(inputs * neuron_layer)
                    args = tuple(args)
                    ...
                    if act_func_for_neurons:
                        args_for_layer['act_func'] = act_func_for_neurons[layer]
                        args_for_layer['deriv_act_func'] = deriv_act_func
                    if weights is not None and len(weights) > layer:
                        args_for_layer['weights'] = weights[layer]
                    args_for_layer['neurons_in_layer'] = neuron_layer
                    args_for_layer['inputs'] = inputs
                    inputs = neuron_layer
                    neurons.append(model_for_neurons(eta, alpha, args_for_layer))
            else:
    ...
    def __update_weights__(self, x, y, results, args):
        y = FeedForwardNetwork.__convert_label_to_result__(y, results.shape[1]).to_numpy()
        results = np.array(
            [FeedForwardNetwork.__j2_loss_function__(result, self.low, self.high) for result in results[0]])

        deltas = []
        # Calculating deltas
        # Calculating delta for output layer
        delta = self.neurons[self.layers-1].calc_delta_func({"is_hidden_layer": False, 'actual': y, 'predicted': results})
        deltas.append(delta)

        # For hidden layers
        for neuron_layers, upper_layer in zip(self.neurons[-2::-1], self.neurons[-1:0:-1]):
            weights = upper_layer.get_weighted_delta()
            args = {"is_hidden_layer": True, 'w_i_j': weights}
            delta = neuron_layers.calc_delta_func(args)
            deltas.append(delta)
        deltas.reverse()
        ...
        for layer in range(self.layers-1, -1, -1):
            weights[layer] = self.neurons[layer].__update_weights__(args={'delta': deltas[layer]})
    ...
    def __get_index_of_highest_output__(result: pd.Series):
        ...
        return np.where(result == np.max(result))[0][0]
    ...
    def __predict__(self, input_layer, args=None):
        input_layer = input_layer.to_numpy().reshape(1, input_layer.shape[0])
        output = None
        for layer in self.neurons:
            output = layer.predict(input_layer, args)
            input_layer = output
        return output
    ...
    def __train__(self, x, y, args):
        if not args:
            args = dict()
        if type(x) == pd.Series:
            return self.__train_layers__(x.to_numpy(), args)
    ...
    def __train_layers__(self, input_layer, args):
        ...
        output = None
        for layer in self.neurons:
            output = layer.__train__(input_layer, None, args)
            input_layer = output
        return output
    ...
    def print_weights(self):
        ...
        for layer, neuron_layer in enumerate(self.neurons):
            print(f"\nWeights of the neuron at Layer {layer}: {neuron_layer.get_weights()}")
            # for index, neuron in enumerate(neuron_layer):
            #     print(f"Weights of the neuron at index {index}:\n {neuron.get_weights()}")
    ...
    def get_weights(self):
        return [layer.w for layer in self.neurons]
...
class AutoEncoderClassifier(FeedForwardNetwork):
    ...
    def __update_weights__(self, x, y, results, args):
        ...
        # Updating weights
        weights = {1: self.neurons[1].__update_weights__(args={'delta': deltas[0]}), 0: self.neurons[0].get_weights()}
        return weights
