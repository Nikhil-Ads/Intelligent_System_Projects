...
def calculate_error_on_train_test(epoch, args):
    ...
    if epoch+1 > 0:
        print(f"Time taken in Epoch {epoch+1}: ", (datetime.datetime.now() - model['start_time']).seconds
              , ' sec')
    x_train = model['x_train']
    y_train = model['y_train']
    if 'y_pred' in model:
        y_train_pred = pd.Series(model['y_pred'][epoch], index= y_train.index).apply(lambda y: pd.Series(y[0]))
    else:
        y_train_pred = args['model'].predict(x_train, {}).apply(lambda y: pd.Series(y[0]))
...
if __name__ == '__main__':
    ...
    feedFwdNwk = FeedForwardNetwork(layers=layers, neurons_in_layers=neurons_in_layers,
                                    ...
                                    eta=0.05, alpha=0.05, weights=weights)
    ...
    if train:
        ...
        weights = feedFwdNwk.train(X_train, X_train, epochs=300,
                                   ...
                                   epoch_pre_process=before_epoch, epoch_pre_process_args={'frac': 1.0,
                                                                                           'x_train': X_train,
                                                                                           'x_index': indexes,
                                                                                           'y_train': X_train,
                                                                                           'y_index': indexes},
    ...
    lms_i_over_train = []
    for i in range(10):
        x_train_label = X_train.loc[Y_train[Y_train['Label'] == i].index.to_list()]
        y_pred = feedFwdNwk.predict(x_train_label, {}).apply(lambda y: pd.Series(y[0]))
    ...
    lms_i_over_test = []
    for i in range(10):
        x_test_label = X_test.loc[Y_test[Y_test['Label'] == i].index.to_list()]
        y_pred = feedFwdNwk.predict(x_test_label, {}).apply(lambda y: pd.Series(y[0]))
    ...
    x_test_outputs = feedFwdNwk.predict(x_test_inputs, {}).apply(lambda y: pd.Series(y[0]))
